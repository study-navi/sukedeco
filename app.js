...
document.addEventListener('DOMContentLoaded',()=>{
 ...
 let drag=false,sx=0,sy=0,ox=0,oy=0; const layer=$('illustLayer'); layer.addEventListener('dragstart',e=>e.preventDefault()); layer.addEventListener('pointerdown',e=>{e.preventDefault();drag=true;sx=e.clientX;sy=e.clientY;ox=state.illustX;oy=state.illustY;layer.classList.add('dragging');layer.setPointerCapture(e.pointerId);}); layer.addEventListener('pointermove',e=>{if(!drag)return; state.illustX=ox+(e.clientX-sx); state.illustY=oy+(e.clientY-sy); renderIllust();}); layer.addEventListener('pointerup',()=>{drag=false; layer.classList.remove('dragging'); saveState();}); layer.addEventListener('pointercancel',()=>{drag=false; layer.classList.remove('dragging');});
 load();
});
